...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunc = function calcFunc() {
    ...
    if (sizeBlock.value == '' || materialBlock.value == '') {
      ...
      resultBlock.textContent = Math.round(sim * 0.7);
    } else {
...
var modals = function modals() {
  var btnRessed = false; // эта переменная будет ловить нажал ли пользователь на какуюто кнопку или нет
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', function () {
      if (!btnRessed && window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight) {
        // условие для того чтобы определить сколько пролистал пользователь на странице вниз
